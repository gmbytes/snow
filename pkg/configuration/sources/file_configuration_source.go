...
const fileReloadDebounceDelay = 500 * time.Millisecond
		...
		go func() {
			...
			for {
				...
				case event, ok := <-watcher.Events:
					...
					if event.Has(fsnotify.Write | fsnotify.Create) {
						if pathEquals(event.Name, ss.path) && atomic.CompareAndSwapInt32(&uniqueRead, 0, 1) {
							...
							go func() {
								<-time.After(fileReloadDebounceDelay)
				...
				case watchErr, ok := <-watcher.Errors:
					if watchErr != nil {
						log.Println("file watcher error:", watchErr)
					}
					if !ok {
						return
					}
...
func pathEquals(path1, path2 string) bool {
	p1, p2 := strings.ReplaceAll(path1, "\\", "/"), strings.ReplaceAll(path2, "\\", "/")
	return p1 == p2
}
