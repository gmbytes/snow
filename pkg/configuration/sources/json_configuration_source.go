...
func ConvertJsonToConfigurationKV(head, json string) (*configuration.CaseInsensitiveStringMap[string], error) {
	var jsons map[string]any
	if err := xjson.UnmarshalFromString(json, &jsons); err != nil {
		return nil, fmt.Errorf("json unmashal failed: %w", err)
	}
