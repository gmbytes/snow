...
//nolint:gochecknoinits // version and build time must be initialized at package load
func init() {
...
func prereleaseGreaterThan(lhs, rhs string) bool {
	if rhs == "" {
		return false
	}
	...
	return len(lhsSplits) >= len(rhsSplits)
}

func identifierCompare(lhs, rhs string) int {
