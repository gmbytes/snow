...
const (
	logNodeNameMaxLen  = 26
	logIDMaxLen        = 12
	logTimeCentisecond = 10
)
...
func DefaultLogFormatter(logData *LogData) string {
	...
	sb.WriteString(fmt.Sprintf(
		...
		year, mon, day,
		hour, m, sec,
		now.Nanosecond()/1000/1000/logTimeCentisecond,
	))
	if len(logData.NodeName) > logNodeNameMaxLen {
	...
	if logData.ID == "" {
		sb.WriteString("            -")
	} else if len(logData.ID) > logIDMaxLen {
