...
func (ss *writer) loop(c <-chan *writerElement) {
	for {
		...
		_ = os.MkdirAll(dir, logDirPerm)

		f, err := os.OpenFile(unit.File, os.O_APPEND|os.O_CREATE|os.O_WRONLY, logFilePerm)
		if err != nil {
		...
		zipFile, err := os.OpenFile(previousName+".zst", os.O_CREATE|os.O_TRUNC|os.O_WRONLY, logFilePerm)
		if err != nil {
