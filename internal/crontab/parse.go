...
func Parse(cronLine string) (*CronExpression, error) {
	...
	if fieldCount < cronMinFieldCount {
		return nil, fmt.Errorf("missing field(s)")
	}

	if fieldCount > cronMaxFieldCount {
		fieldCount = cronMaxFieldCount
	}
...
func (c *CronExpression) parseFieldEntry(entry string, desc fieldDescriptor) (*timeRange, error) {
	...
	if idx := strings.IndexByte(entryLower, '/'); idx > 0 {
		...
		if dashIdx := strings.IndexByte(valueStr, '-'); dashIdx > 0 {
			...
			begin, beginErr := c.parseValue(beginStr, desc)
			if beginErr != nil {
				return nil, beginErr
			}
			end, endErr := c.parseValue(endStr, desc)
			if endErr != nil {
				return nil, endErr
			}
			r.begin = begin
		...
		begin, beginErr := c.parseValue(valueStr, desc)
		if beginErr != nil {
			return nil, beginErr
		}
		r.begin = begin
...
func (*CronExpression) parseValue(s string, desc fieldDescriptor) (int, error) {
	// 尝试直接解析为数字
